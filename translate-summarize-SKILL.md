...
将 AI 入选文章的来源元数据与受限 DOM 文本快照转换为中文标题、摘要、时间判断和质量结论，输出供本地 Obsidian 发布流程读取的结构化 JSON。
...
默认仅对入选项读取受限长度的 DOM 文本快照。快照保存在本地运行报告中供失败重试复用，不作为完整正文写入文章笔记。

### Step 3: 快照处理与错误隔离

由 `scripts/llm_summarize.py` 中的 `process_selected_snapshots` 执行：

1. **小批次调用**：默认每批 3 条，一次完成翻译、摘要、时间确认和质量判断。
2. **二分降级容错**：若某批次解析或 LLM 返回失败，自动进行递归二分隔离，确保单篇异常文章不会拖垮整批任务。
3. **极简模式保底**：对极端失败的条目缩短快照进行单篇最小上下文重试。
