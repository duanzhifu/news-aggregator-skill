...
这个项目会抓取多个新闻、技术和社交平台来源。Obsidian 日报由 AI 解释各平台时间、选择值得打开的候选，再依据受限页面文本快照完成翻译、总结和质量确认。它也可以只输出原始 JSON，供其他程序继续处理。
...
| `--evidence-mode snapshot` | 默认；只对 AI 入选项读取受限 DOM 文本快照 |
| `--evidence-mode full` / `--deep` | 只对 AI 入选项抓取完整正文，耗时与失败率更高 |
...
| `weibo_search` | 微博 | 可配置 JSON API 或公开搜索页；无关键词时使用热搜接口 |
| `wechat` | 微信公众号 | 可配置 JSON API 优先，默认通过搜索引擎发现公开文章 |
...
python scripts/fetch_news.py `
  --source douyin,bilibili,weibo_search,wechat `
  --keyword "前端,AI,软件工程" `
  --limit 5 `
  --no-save
```
...
未配置接口或接口调用失败时，程序会降级至 Playwright 浏览器抓取。首次使用登录态时，运行交互式初始化命令；浏览器打开后自行完成抖音、B站和微博登录，最后回到终端按 Enter 保存会话：
...
python scripts/fetch_news.py --source douyin,bilibili,weibo_search --keyword "AI" --limit 1 --no-save
```

`run_daily.ps1` 会自动使用上述默认 Profile 和 Microsoft Edge；目录不存在时记录警告并降级为临时 Edge 会话。登录过期后重新运行初始化命令。微信公众号仍通过公开搜索发现文章，不依赖微信个人登录。程序不会读取日常 Edge Profile、保存明文密码、自动处理验证码或绕过访问限制。
...
| 脚本 | 用途 |
| --- | --- |
| `scripts/run_daily.ps1` | 常规日报，来源为 `juejin,devto,github,openai,douyin,bilibili,weibo_search,wechat`，每源 15 条 |
